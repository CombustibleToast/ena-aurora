// Aurora's Song - Copied from SalvageExpeditionConsoleBoundUserInterface.cs originally written by Wizard's Den for Space Station 14
using System.Linq;
using Content.Client._AS.SLE.UI; // Aurora's Song
using Content.Client.Stylesheets;
using Content.Shared.CCVar;
using Content.Shared.Procedural;
using Content.Shared.Salvage.Expeditions;
using Content.Shared.Salvage.Expeditions.Modifiers;
using Content.Shared._AS.SLE.Events;
using Content.Shared._AS.SLE.Mission;
...
namespace Content.Client._AS.SLE.UI;

[UsedImplicitly]
public sealed class SLEMissionConsoleBoundUserInterface : BoundUserInterface
{
    [ViewVariables]
    private SLEMissionWindow? _window; // Aurora's Song - OfferingWindow<SLEMissionWindow>
    ...
    public SLEMissionConsoleBoundUserInterface(EntityUid owner, Enum uiKey) : base(owner, uiKey)
    {
    ...
    {
        base.Open();
        _window = this.CreateWindowCenteredLeft<SLEMissionWindow>(); // Aurora's Song - OfferingWindow<SLEMissionWindow>
        _window.Title = Loc.GetString("salvage-expedition-window-title");
        _window.OnFinishPressed += () => SendMessage(new FinishSLEMissionMessage()); // Aurora's Song
    }
    ...
    protected override void UpdateState(BoundUserInterfaceState state)
    {
        ...
        if (state is not SLEMissionConsoleState current || _window == null)
            return;
        ...
        var salvage = _entManager.System<SLEMissionSystem>();
